...
with open(
    "./models/pkl/binary-fine-grained-sentiment/sentiment_model.pkl",
    "rb",
) as f:
...
with open(
    "./models/pkl/count_vectorizer.pkl",
    "rb",
) as f:
    count_vect = pickle.load(f)

# Load the saved tfidf transformer
with open("./models/pkl/tfidf_transformer.pkl", "rb") as f:
...
def text_sentiment_preprocessing(
    text, language, minWordLength, model, count_vect, tfidf_transformer, label_encoder
):
...
def get_binary_sentiment(
    text, language, minWordLength, model, count_vect, tfidf_transformer, label_encoder
):
    predicted_label, predicted_amount = text_sentiment_preprocessing(
        text=text,
        language=language,
...
def get_fine_grained_sentiment(
    text, language, minWordLength, model, count_vect, tfidf_transformer, label_encoder
):
    predicted_label, predicted_amount = text_sentiment_preprocessing(
        text=text,
    ...
    result = {"text": text, "sentiment": predicted_label, "score": predicted_amount}
...
def get_emotions(text):
    ...
    for result in emotion_prediction[0]:
        emotions.append(
            {"emotion": result["label"], "intensity": round(float(result["score"]), 3)}
        )
...
def get_all_emotions():
    return {"emotions": ["surprise", "sadness", "joy", "love", "anger", "fear"]}
