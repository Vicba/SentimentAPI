      ...
      <table className="w-full">
        ...
        <tbody>
          {keys.map((keyItem) => (
            <tr
              key={keyItem.id}
              className="border-b-slate-300 dark:border-b-gray-200 border-b-2 dark:border-opacity-60 border-opacity-60 flex justify-between"
            >
              <td className="py-3 flex items-center">
                <div className="flex items-center">
                  {showFullKey === keyItem.id
                    ? keyItem.key
                    : `${keyItem.key.substring(0, keyItem.key.length / 2)}...`}
                  {showFullKey === keyItem.id ? (
                    <EyeIcon
                      height={20}
                      width={20}
